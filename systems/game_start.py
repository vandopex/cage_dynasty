...
COACH_PERSONALITIES = [
    ("Patient", "Good with young fighters who need fundamentals"),
    ("Intense", "Pushes fighters hard, fast results but risk of burnout"),
    ("Technical", "Focuses on technique over power"),
    ("Motivator", "Excellent at building confidence and mental game"),
    ("Methodical", "Slow and steady development approach"),
    ("Demanding", "High standards, won't accept mediocrity"),
    ("Supportive", "Creates positive training environment"),
    ("Old School", "Traditional training methods, proven results"),
]
...
class StartingCoach:
    ...
    # Personality
    personality_trait: str = "Patient"
    personality_desc: str = "Good with young fighters"
    ...
    # Runtime attributes (set when assigned to a camp)
    camp_id: str = ""
    is_head_coach: bool = False
    
    # Compatibility with Coach class - derived quality from skill_level
    @property
    def quality(self) -> int:
        """Return quality in Coach-compatible format (20-100 scale)."""
        return min(100, max(20, self.skill_level))
    
    @property
    def traits(self) -> List[str]:
        """Return traits list for compatibility with Coach class."""
        return [self.personality_trait] if self.personality_trait else []
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fight_bonus": self.fight_bonus,
            "camp_id": self.camp_id,
            "is_head_coach": self.is_head_coach,
            "_type": "StartingCoach",  # Type marker for deserialization
        }
    
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "StartingCoach":
        """Deserialize from dictionary."""
        coach = cls(
            coach_id=data.get("coach_id", ""),
            name=data.get("name", "Coach"),
            specialty=data.get("specialty", "Boxing"),
            skill_level=data.get("skill_level", 70),
            personality_trait=data.get("personality_trait", "Patient"),
            ...
            annual_cost=data.get("annual_cost", 78000),
            training_bonus=data.get("training_bonus", 0.10),
            fight_bonus=data.get("fight_bonus", 0.05),
        )
        coach.camp_id = data.get("camp_id", "")
        coach.is_head_coach = data.get("is_head_coach", False)
        return coach


# Coach trait descriptions for display
COACH_TRAIT_DESCRIPTIONS = {
    # Starting coach personalities (from COACH_PERSONALITIES) - MUST MATCH
    "Patient": "Good with young fighters who need fundamentals",
    "Intense": "Pushes fighters hard, fast results but risk of burnout",
    "Technical": "Focuses on technique over power",
    "Motivator": "Excellent at building confidence and mental game",
    ...
    # Additional traits (from coaches.py)
    "Technical Genius": "+15% technique training effectiveness",
    ...
    "Disciplined": "Consistent training results",
    "Encouraging": "Maintains fighter confidence",
    "Strategic": "Better fight preparation",
    "Adaptable": "Adjusts training to fighter needs",
    ...
    "Impatient": "May clash with slow learners",
    "Harsh": "Effective but hurts morale",
}


def get_coach_trait_description(trait: str) -> str:
    """Get description for a coach personality trait."""
    return COACH_TRAIT_DESCRIPTIONS.get(trait, "Unique coaching style")
...
def generate_starting_prospects(
    ...
    for i, weight_class in enumerate(WEIGHT_CLASSES[:num_prospects]):
        ...
        # Age (younger for higher potential) - max 23 for all prospects
...
def generate_starting_coaches(num_coaches: int = 10) -> List[StartingCoach]:
    ...
    for i in range(num_coaches):
        ...
        # Personality
        personality = random.choice(COACH_PERSONALITIES)
        ...
        coach = StartingCoach(
            ...
            specialty=specialty,
            skill_level=skill,
            personality_trait=personality[0],
            personality_desc=personality[1],
