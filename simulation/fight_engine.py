...
# ============================================================================
# V7 BALANCE PARAMETERS - UFC-REALISTIC FINISH RATES
# ============================================================================
# Target: 35-40% KO/TKO, 10-15% SUB, 45-55% DEC
# These values tuned through extensive testing

# Damage scaling - applied to all strike damage
DAMAGE_MULTIPLIER = 0.45  # Balanced for ~40% finish rate without per-exchange healing

# Flash KO System - Sudden knockouts from big shots
FLASH_KO_DAMAGE_THRESHOLD = 7.0  # Min damage to trigger flash KO check
FLASH_KO_BASE_CHANCE = 0.002     # 0.2% per damage point above threshold
FLASH_KO_MAX_CHANCE = 0.10       # 10% cap

# TKO (Ground and Pound) System - Referee stoppages from dominant positions
TKO_GNP_HEALTH_THRESHOLD = 35.0  # Health below which TKO possible
TKO_GNP_BASE_CHANCE = 0.02       # 2% base chance when conditions met
TKO_GNP_MAX_CHANCE = 0.10        # 10% cap

# TKO Standing System - Referee stops fight when hurt on feet
TKO_STANDING_HEALTH_THRESHOLD = 30.0  # Very hurt threshold
TKO_STANDING_BASE_CHANCE = 0.03       # 3% base when rocked + low health

# BJJ Thresholds - Submission success bonuses
BJJ_ELITE_THRESHOLD = 85    # Elite BJJ bonus starts here
BJJ_GOOD_THRESHOLD = 80     # Good BJJ bonus starts here
...
class FightConfig:
    ...
    BALANCE NOTES (December 2024):
    Target: ~50% finish rate (real MMA ~45-55%)
    Formula: (exchanges Ãƒâ€” damage_mult Ãƒâ€” 3 rounds) / health = ~0.60-0.65
    
    CURRENT: 65 Ãƒâ€” 0.35 Ãƒâ€” 3 = 68.25 / 120 = 0.57 ratio
    ...
    exchanges_per_round: int = 55  # Exchanges per round
    
    damage_multiplier: float = DAMAGE_MULTIPLIER  # Uses global V7 constant
    standup_threshold: int = 6  # Balanced for ground time
    doctor_check_cut_threshold: int = 2
    
    submission_progress_to_finish: float = 80.0  # Moderate sub difficulty
    submission_escape_threshold: float = 85.0  # Escapes possible but hard
...
def get_available_strikes(position: Position, is_top: bool = True) -> List[StrikeType]:
    ...
    # === LEG ENTANGLEMENTS - No strikes, pure grappling ===
    if position in LEG_ENTANGLEMENT_POSITIONS:
        return []
    
    # === TRUCK - Limited striking ===
    if position == Position.TRUCK:
        if is_top:
            return [StrikeType.GNP_PUNCH]
        return []
    
    # === BACK MOUNT - Punches to side of head only ===
    # Can't elbow effectively, focus is on choke
    if position == Position.BACK_MOUNT:
        if is_top:
            return [StrikeType.GNP_PUNCH, StrikeType.GNP_HAMMER_FIST]
        return []  # Can't strike when hooks are in
    
    if position == Position.BACK_MOUNT_BOTTOM:
        return []  # Survival mode - no strikes
    
    # === TURTLE - GNP from top ===
    if position == Position.TURTLE_TOP:
        return [StrikeType.GNP_PUNCH, StrikeType.GNP_HAMMER_FIST, StrikeType.CLINCH_KNEE]
    
    if position == Position.TURTLE_BOTTOM:
        return []  # Survival mode
    
    # === SPRAWL - Knees from top ===
    if position == Position.SPRAWL:
        if is_top:
            return [StrikeType.CLINCH_KNEE]
        return []
    
    # === STANDING BACK CONTROL - Limited punches ===
    if position == Position.STANDING_BACK:
        if is_top:
            return [StrikeType.GNP_PUNCH]  # Short punches to side of head
        return []
    
    # === KNOCKDOWN STANDING - Soccer kicks and GNP ===
    if position == Position.KNOCKDOWN_STANDING:
        if is_top:
            return [
                StrikeType.GNP_PUNCH, StrikeType.GNP_HAMMER_FIST,
                StrikeType.LEG_KICK  # Soccer kicks
            ]
        return [StrikeType.ELBOW_UPWARD]  # Upkicks from ground
    
    # === AGAINST CAGE GROUND ===
    if position == Position.AGAINST_CAGE_GROUND:
        if is_top:
            return [StrikeType.GNP_PUNCH, StrikeType.GNP_HAMMER_FIST, StrikeType.GNP_ELBOW]
        return [StrikeType.ELBOW_UPWARD]
    
    # === DOMINANT GROUND POSITIONS (top) ===
    if is_top:
        ...
        # Guard positions from top - GNP but limited by guard
        elif position in {Position.FULL_GUARD_TOP, Position.HALF_GUARD_TOP,
                         Position.CLOSED_GUARD_TOP, Position.BUTTERFLY_GUARD_TOP}:
            return [StrikeType.GNP_PUNCH, StrikeType.GNP_ELBOW]
    
    # === INFERIOR POSITIONS (bottom) ===
    if not is_top:
        # Guard bottom - can throw elbows upward
        if position in {Position.FULL_GUARD_BOTTOM, Position.HALF_GUARD_BOTTOM,
                       Position.CLOSED_GUARD_BOTTOM, Position.BUTTERFLY_GUARD_BOTTOM,
                       Position.RUBBER_GUARD_BOTTOM}:
            return [StrikeType.ELBOW_UPWARD]
        # Truly inferior positions - no effective strikes
        if position in {Position.SIDE_CONTROL_BOTTOM, Position.MOUNT_BOTTOM,
                       Position.NORTH_SOUTH_BOTTOM, Position.CRUCIFIX_BOTTOM}:
            return []  # Too pinned to strike
    
    # === TAKEDOWN ATTACK POSITIONS - No strikes ===
    if position in {Position.SINGLE_LEG_ATTACK, Position.DOUBLE_LEG_ATTACK}:
        return []
...
def get_available_grappling_actions(
    ...
    # Knockdown standing - attacker over downed opponent
    elif position == Position.KNOCKDOWN_STANDING:
        if is_top:
            # Top fighter can dive into guard or let opponent up
            actions.extend([
                GrapplingAction.PASS_TO_SIDE,  # Dive to side control
                GrapplingAction.PASS_TO_HALF,  # Follow into half guard
            ])
        else:
            # Bottom fighter tries to recover
            actions.extend([
                GrapplingAction.STAND_UP,      # Technical standup
                GrapplingAction.REGUARD,       # Pull into guard
            ])
        ...
        # === NEW: NORTH_SOUTH_TOP - Can transition to mount or back ===
        elif position == Position.NORTH_SOUTH_TOP:
            actions.extend([
                GrapplingAction.MOUNT_TRANSITION,
                GrapplingAction.TAKE_BACK,
            ])
        # === NEW: CRUCIFIX_TOP - Very dominant, limited transitions ===
        elif position == Position.CRUCIFIX_TOP:
            actions.append(GrapplingAction.MOUNT_TRANSITION)
        # === NEW: STANDING_BACK - Standing back control ===
        elif position == Position.STANDING_BACK:
            actions.extend([
                GrapplingAction.BODY_LOCK_TAKEDOWN,  # Drag to ground
                GrapplingAction.SUPLEX,              # Throw
            ])
            if fighter_attrs.takedowns >= 70:
                actions.append(GrapplingAction.HIP_TOSS)
        # === NEW: AGAINST_CAGE_GROUND (top) ===
        elif position == Position.AGAINST_CAGE_GROUND:
            actions.extend([
                GrapplingAction.PASS_TO_SIDE,
                GrapplingAction.PASS_TO_HALF,
            ])
        # === NEW: BUTTERFLY_GUARD_TOP - Pass attempts ===
        elif position == Position.BUTTERFLY_GUARD_TOP:
            actions.extend([
                GrapplingAction.PASS_TO_SIDE, GrapplingAction.PASS_TO_HALF,
                GrapplingAction.TORREANDO
            ])
        ...
        if position in {Position.FULL_GUARD_BOTTOM, Position.CLOSED_GUARD_BOTTOM}:
            ...
            # More escape options from back mount bottom
            actions.extend([
                GrapplingAction.GRANBY_ROLL,
                GrapplingAction.BRIDGE_ESCAPE,  # Try to dislodge hooks
            ])
            if fighter_attrs.guard >= 60:
                actions.append(GrapplingAction.REGUARD)  # Skilled fighters can reguard
        # Handle BACK_MOUNT with is_top=False (actual game state uses this)
        elif position == Position.BACK_MOUNT:
            # Bottom fighter in back mount - escape attempts
            actions.extend([
                GrapplingAction.GRANBY_ROLL,
                GrapplingAction.BRIDGE_ESCAPE,
            ])
            if fighter_attrs.guard >= 60:
                actions.append(GrapplingAction.REGUARD)
        elif position == Position.TURTLE_BOTTOM:
            actions.extend([
                GrapplingAction.GRANBY_ROLL, GrapplingAction.STAND_UP
            ])
        # === NEW: NORTH_SOUTH_BOTTOM - Escape attempts ===
        elif position == Position.NORTH_SOUTH_BOTTOM:
            actions.extend([
                GrapplingAction.SHRIMP_ESCAPE,
                GrapplingAction.REGUARD,
                GrapplingAction.BRIDGE_ESCAPE,
            ])
        # === NEW: CRUCIFIX_BOTTOM - Very bad position, limited escapes ===
        elif position == Position.CRUCIFIX_BOTTOM:
            actions.extend([
                GrapplingAction.BRIDGE_ESCAPE,  # Desperate escape attempt
            ])
        # === NEW: RUBBER_GUARD_BOTTOM - Active guard position ===
        elif position == Position.RUBBER_GUARD_BOTTOM:
            actions.extend([
                GrapplingAction.SCISSOR_SWEEP, GrapplingAction.STAND_UP
            ])
            if fighter_attrs.guard >= 70:
                actions.append(GrapplingAction.ENTER_SINGLE_LEG_X)
        # === NEW: AGAINST_CAGE_GROUND (bottom) ===
        elif position == Position.AGAINST_CAGE_GROUND and not is_top:
            actions.extend([
                GrapplingAction.STAND_UP,
                GrapplingAction.REGUARD,
            ])
...
def select_action(
    ...
    if position in STANDING_POSITIONS:
        ...
        # === SPECIAL: BACK_MOUNT - RNC hunting position ===
        # Back mount is THE most dangerous position in MMA
        # Fighters should primarily hunt for the rear naked choke
        if position == Position.BACK_MOUNT:
            if position_secured:
                # From back mount, submissions are HEAVILY prioritized
                if fighter_attrs.submissions >= 85:
                    sub_weight += 350  # Elite - constant RNC threat
                elif fighter_attrs.submissions >= 75:
                    sub_weight += 200  # Good - hunting the choke
                elif fighter_attrs.submissions >= 65:
                    sub_weight += 100  # Decent - can attempt
                else:
                    sub_weight += 40   # Even non-grapplers try from here
            # Back mount strikes are limited - punches to side of head
            strike_weight += 20  # REDUCED from 50 - less effective strikes
            grapple_weight += 5   # Can transition to truck
        else:
            # Other dominant positions (mount, side control, etc.)
            if position_secured:
    ...
    if total == 0:
        # No weighted actions available - use what's available
        # Priority: grappling (escapes) > strikes > defensive action
        if grappling:
            action = random.choice(grappling)
            return ("grappling", action)
        elif strikes:
            return ("strike", random.choice(strikes))
        else:
            # Truly no options - this shouldn't happen but return a defensive grapple
            return ("grappling", GrapplingAction.STAND_UP)
...
def calculate_strike_success(
    ...
    variance = random.uniform(0.75, 1.25)  # Ã‚Â±25% variance (was Ã‚Â±15%)
    offense *= variance
...
def simulate_exchange(
    ...
    if action_type == "strike":
        ...
        if landed:
            ...
            if (not is_finish and target == "head" and 
                ...
                # Hurt bonus (defender already rocked or low health)
            ...
            if (not is_finish and 
                fight_state.top_fighter_id == attacker.fighter_id and
                defender_state.health < TKO_GNP_HEALTH_THRESHOLD and
                fight_state.position in DOMINANT_POSITIONS):
            ...
            if (not is_finish and
                ...
                fight_state.position in STANDING_POSITIONS):
