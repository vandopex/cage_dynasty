...
class NarratedFightSimulator:
    ...
    def _execute_strike(
        ...
        if landed:
            ...
            # Apply config damage multiplier (centralized in fight_engine.py)
            ...
            # Check for rock
            if damage > 12 and not caused_knockdown:
                if random.random() < 0.3:
                    defender_state.is_rocked = True
                    defender_state.rock_duration = 2
                    caused_rock = True
            
            # Check for finish
            if is_finish:
                method = "KO" if caused_knockdown else "TKO"
