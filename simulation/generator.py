...
Usage:
    ...
    style = generate_fighting_style(country="Brazil", attributes={"submissions": 85, "takedowns": 60})
...
REGIONAL_TRAITS: Dict[str, Dict] = {
    ...
    "South America": {
        "attr_mod": {"submissions": 4, "guard": 2, "composure": 2},
        "style_tendency": "bjj_specialist"
    },
    "Eastern Europe": {
        "attr_mod": {"takedowns": 3, "top_control": 2, "strength": 2, "chin": 2},
        "style_tendency": "sambo"
    },
    "Central Asia": {
        "attr_mod": {"takedowns": 5, "top_control": 3, "cardio": 2, "heart": 2},
        "style_tendency": "wrestler"
    },
    "North America": {
        "attr_mod": {"takedowns": 2, "boxing": 2},
...
def select_style_for_location(city: str, country: str) -> Dict[str, Any]:
    """
    Select a fighting style using city-level culture overrides where available,
    falling back to country-level COUNTRY_STYLE_MAP.

    A camp in Phuket generates ~80% Muay Thai fighters.
    A camp in Makhachkala generates ~85% wrestlers/sambo fighters.
    A generic city uses the country default.
    """
    try:
        from simulation.camp_generator import get_location_style_weights
    except ImportError:
        try:
            from camp_generator import get_location_style_weights
        except ImportError:
            return select_style_for_country(country)

    style_weights = get_location_style_weights(city, country)
    ...
    for style in STYLES:
        if style["name"] == selected_name:
            return style
    # Partial match (e.g. "BJJ" matches "BJJ Specialist" style name)
    for style in STYLES:
        if selected_name.lower() in style["name"].lower():
            return style
    return select_style_for_country(country)
...
def _determine_style_from_attributes(attributes: Dict[str, int]) -> FightingStyle:
    ...
    takedowns = attributes.get("takedowns", 50)
    top_control = attributes.get("top_control", 50)
    submissions = attributes.get("submissions", 50)
    guard = attributes.get("guard", 50)
    ...
    wrestling_score = (takedowns + top_control) / 2
    bjj_score = (submissions + guard) / 2
    grappling_score = (wrestling_score + bjj_score) / 2
    
    # High submissions, moderate takedowns -> BJJ_SPECIALIST
    if bjj_score >= 75 and bjj_score > wrestling_score:
        return FightingStyle.BJJ_SPECIALIST
    
    # High takedowns/control -> WRESTLER or GROUND_AND_POUND
    ...
    if clinch_striking >= 70 and takedowns >= 60:
...
def generate_fighter(
    ...
    Args:
        ...
        city: Camp city — drives style selection when provided
        weight_class: Target weight class (random if None)
        weight: Specific weight (generated if None)
        style: Fighting style name (city/country-based if None)
    ...
    if style:
        ...
        if style_data is None:
            style_data = select_style_for_location(city or "", country) if city else select_style_for_country(country)
    elif city:
        style_data = select_style_for_location(city, country)
    else:
        style_data = select_style_for_country(country)
    ...
    recovery = stat_gauss(base_mu, sigma)  # NEW: Between-round recovery
    ...
    # Grappling stats (5) - NEW SPLIT
    takedowns = stat_gauss(base_mu, sigma)        # Getting fight to ground
    takedown_defense = stat_gauss(base_mu, sigma)
    top_control = stat_gauss(base_mu, sigma)      # Holding position, GnP
    submissions = stat_gauss(base_mu, sigma)       # Finishing ability
    guard = stat_gauss(base_mu, sigma)             # Sweeps, escapes
    ...
    if region and region in REGIONAL_TRAITS:
        ...
        for attr, mod in traits.items():
            if attr == "speed":
                ...
                recovery = max(1, min(100, recovery + mod))
            elif attr == "kicks":
                kicks = max(1, min(100, kicks + mod))
            elif attr == "takedowns":
                takedowns = max(1, min(100, takedowns + mod))
            elif attr == "top_control":
                top_control = max(1, min(100, top_control + mod))
            elif attr == "submissions":
                submissions = max(1, min(100, submissions + mod))
            elif attr == "guard":
                guard = max(1, min(100, guard + mod))
            elif attr == "boxing":
    ...
    takedowns = max(1, min(100, takedowns + style_mods.get("takedowns", 0)))
    top_control = max(1, min(100, top_control + style_mods.get("top_control", 0)))
    submissions = max(1, min(100, submissions + style_mods.get("submissions", 0)))
    guard = max(1, min(100, guard + style_mods.get("guard", 0)))
    ...
    if random.random() < 0.003:
        ...
        skill_attrs = [
            ...
            ('submissions', submissions), ('striking_defense', striking_defense),
            ('top_control', top_control), ('guard', guard)
        ]
        ...
        for attr_name, _ in skill_attrs[:3]:
            ...
            elif attr_name == 'takedowns': takedowns = min(99, takedowns + boost)
            elif attr_name == 'submissions': submissions = min(99, submissions + boost)
            elif attr_name == 'striking_defense': striking_defense = min(99, striking_defense + boost)
            elif attr_name == 'top_control': top_control = min(99, top_control + boost)
            elif attr_name == 'guard': guard = min(99, guard + boost)
...
def get_fighter_summary(fighter: FighterAttributes) -> str:
    ...
    overall = (
        fighter.boxing + fighter.kicks + wrestling_ability + 
        bjj_ability + fighter.strength + fighter.speed +
        fighter.chin + fighter.heart
    ) // 8
    
    if fighter.boxing > wrestling_ability and fighter.boxing > bjj_ability:
        tendency = "Striker"
    elif wrestling_ability > bjj_ability:
        tendency = "Wrestler"
    elif bjj_ability > wrestling_ability:
        tendency = "Grappler"
    else:
...
def calculate_overall(fighter: FighterAttributes) -> int:
    """Calculate overall rating for a fighter (17 attributes)"""
    return (
        # Physical (5)
        fighter.strength + fighter.speed + fighter.cardio + 
        fighter.chin + fighter.recovery +
        # Striking (4)
        fighter.boxing + fighter.kicks + fighter.clinch_striking +
        fighter.striking_defense +
        # Grappling (5)
        fighter.takedowns + fighter.takedown_defense + fighter.top_control +
        fighter.submissions + fighter.guard +
        # Mental (3)
        fighter.heart + fighter.fight_iq + fighter.composure
    ) // 17
