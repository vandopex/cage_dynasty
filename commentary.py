...
class FightCommentarySystem:
    ...
    def __init__(self, context: Optional[FightContext] = None):
        ...
        # in R3+ or in the final 10 exchanges of any earlier round.
        # We track whether we've already fired a high-drama peril line this round
        # so we don't get two contradictory lines in the same round.
        self._high_drama_fired_round: int = -1
    ...
    def _generate_hurt_buildup(
        ...
        exchanges_per_round = (
            self.context.exchanges_per_round
            if self.context and self.context.exchanges_per_round
            else 55
        )
        late_in_round = self.current_exchange >= (exchanges_per_round - 10)
        final_round   = self.context and self.current_round >= (self.context.total_rounds or 3)

        if target_health < 10:
            # Critical — about to be finished.
            # High-drama lines ("IS DONE", "SERIOUS TROUBLE") only fire in R3+
            # or in the last 10 exchanges of any round, AND only once per round.
            if not (self.current_round >= 3 or late_in_round or final_round):
                # Downgrade to a mild line instead
                return random.choice([
                    f"{target} is in trouble!",
                    f"{actor} has {target} hurt!",
                    f"The damage is accumulating on {target}!",
                ])
            if self._high_drama_fired_round == self.current_round:
                return ""   # Already fired once this round — no second drama spike
            self._high_drama_fired_round = self.current_round
            templates = [
            ...
            # Very hurt — "legs are GONE" class lines.
            # Gate to R3+ or late in round; only once per round.
            if not (self.current_round >= 3 or late_in_round or final_round):
                return random.choice([
                    f"{target} is taking damage!",
                    f"{actor} is landing clean!",
                ])
            if self._high_drama_fired_round == self.current_round:
                return ""
            self._high_drama_fired_round = self.current_round
            ...
            # Hurt but surviving — mild lines, no gates needed
            templates = [
                f"{target} is in trouble!",
                f"{actor} has {target} hurt!",
