...
class GameState:
    ...
    def _generate_fighter(self, weight_class: str) -> str:
        ...
        # Generate attributes (simplified - would use full generator)
        _FALLBACK_FIRST = [
            ...
            "Khabib","Conor","Max","Dustin","Tony","Justin","Charles","Islam","Beneil",
            "Sean","Paddy","Bobby","Damon","Arnold","Geoff","Drew","Dan","Kevin","Edson",
        ]
        _FALLBACK_LAST = [
            ...
            "Poirier","Gaethje","Ferguson","Chandler","Hooker","Dariush","Tsarukyan","Rakhmonov",
            "Ortega","Holloway","Volkanovski","Jung","Korean","Emmett","Zombie","Kattar",
        ]
        country = "United States"
        first   = __import__('random').choice(_FALLBACK_FIRST)
        last    = __import__('random').choice(_FALLBACK_LAST)
        try:
            from name_database import COUNTRY_NAMES as _ND, get_random_country as _GRC
            _country = _GRC()
            _pool = _ND.get(_country, _ND.get("United States", {}))
            if _pool.get("first") and _pool.get("last"):
                first = __import__('random').choice(_pool["first"])
                last  = __import__('random').choice(_pool["last"])
        except Exception:
            pass
        name = f"{first} {last}"
        ...
        self._fighter_data[fighter_id] = {
            ...
            "country": random.choice(["USA", "Brazil", "Russia", "UK", "Canada"]),
        }
