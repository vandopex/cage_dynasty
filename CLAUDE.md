...
<!-- ARCHIVE4 --> Body (108 lines) moved verbatim to claude/golden_master_oracle.md — Read before any oracle, harness, or GOLDEN MASTER work. The heading above is the rule; the file is the evidence.
...
<!-- ARCHIVE3 --> Next in order (Van's ruling 2026-09-10, resolution recorded 2026-09-14, deploy #6 2026-09-16): (d) resolved into RECENTLY-FOUGHT1 (deferred; player-only caller, negligible) and REMATCH-COOLDOWN1 (CLOSED 2026-09-16 by browser check; shipped 9faeba8, on PA at 46c094f). FOUNDING-ROW2 also CLOSED 2026-09-16 (shipped 46c094f, canonical streak walkers, browser-verified). NEW dockets from deploy #6 browser session: BELT-LINEAGE1, RECAP-STALE1, HISTORY-CAP1, NAME-COLLISION1, DIVISION-GENDER1, DRAW-SEMANTICS1 (upgraded), HARNESS-ENV1, DUMP-PROVENANCE1. Continuing: PROSPECT-BONUS1 instrument rule → PROSPECT-BONUS1 retune. WEEK-AXIS1 (a)+(b)+(c) ON PA at 46c094f (deploy #6, 2026-09-16, API-driven, 10/10 gates PASS, filing claude/deploy6_2026-09-16.md). Filings claude/week_axis1_b_filing_2026-09-10.md + claude/week_axis1_c_filing_2026-09-11.md + claude/deploy4_2026-09-11.md + claude/rematch_cooldown1_filing_2026-09-14.md + claude/deploy5_2026-09-15.md + claude/deploy6_2026-09-16.md. Behind: see the board.
<!-- ARCHIVE3 --> DEPLOY #7 (2026-09-20 PDT, filing claude/deploy7_2026-09-20.md): shipped 25adadd (CHAMPIONS-PAGE1) + 6540d3c (RECORD-BOOK-REPOINT1) to PA at 4e72bab; steps 0–11 PASS; runtime set N=1 (cage_dynasty_web/game_bridge.py); IMPORT-PATH-PROOF confirms cage_dynasty_web/ resolution; DEPLOY-GATE-REF1 (a) invoked; Van's PA browser check PASS 2026-09-20 (deploy7 filing §k); RECORD-BOOK1 + BELT-LINEAGE1 display half closed.
...
- **Ruling (a) (Van, 2026-09-19):** cc commits and deploys only on a sentence Van wrote. Architect conditionals — "if you rule X, then Y" — are not directives; cc waits for the explicit ruling clause. Recommendations and hypothetical fork-descriptions surface options but do not authorize actions. Filing: `claude/deploy7_2026-09-20.md` §h.
- cc's self-report of which tool it used is not evidence. The architect's fold-check (ctrl+o on the collapsed call) is ground truth. Any instruction that hinges on tool choice is verified by expanding the fold, not by asking cc. (deploy7 filing §j, 2026-09-20) Measured 2026-09-20: 0 Grep tool calls across 8 sessions (deploy7 §k); a collapsed 'Searched for N patterns' header is a summary, not evidence of which tool ran.
...
<!-- ARCHIVE4 --> Command block (30 lines) moved verbatim to claude/local_playtest_command.md — Read before any local playtest.
<!-- ARCHIVE4 --> OWED ITEMS CARRIED (MC ODDS 2026-08-19) and the 5c1477d ledger correction (20 lines) moved verbatim to claude/claude_md_archive_2026c.md.
## Certified cell baselines (symmetric skill)
<!-- ARCHIVE4 --> Body (139 lines) moved verbatim to claude/certified_cell_baselines.md — Read before any cell-baseline or symmetric-skill work.
...
- `claude/deploy5_2026-09-15.md` — Deploy #5 filing: REMATCH-COOLDOWN1 to PA at 6d7abc2; all 10 gates PASS; standing procedure upgrade (console warm-check as step 0, file-hash gate as deploy proof); browser check NOT PERFORMED (owed) — subsequently closed at deploy #6 (2026-09-15)
- `claude/deploy6_2026-09-16.md` — Deploy #6 filing: FOUNDING-ROW2 to PA at 46c094f; 10/10 gates PASS with file-hash byte-identical proof; both owed browser checks closed one sitting (FOUNDING-ROW2 verified via streak boards; REMATCH-COOLDOWN1 verified via Tsarukyan vs Bailey CD62↔CD49 rematch); seven new dockets filed (BELT-LINEAGE1, RECAP-STALE1, HISTORY-CAP1, NAME-COLLISION1, DIVISION-GENDER1, HARNESS-ENV1, DUMP-PROVENANCE1) plus DRAW-SEMANTICS1 upgraded (2026-09-16)
- `claude/handoff_post_deploy6.md` — post-deploy #6 handoff: anchors (no HEAD), one owed ruling, one landed correction, sequencing rec, cc-reading patterns, architect errors (2026-09-16)
- `claude/handoff_post_slice1.md` — post-slice-1 handoff: anchors (no HEAD), rulings landed, sequencing through slice 3, cc-reading patterns, architect errors; §6 corrections layer (provenance, :4388, ruling (a), runtime set). Supersedes handoff_post_deploy6 (2026-09-18)
- `claude/deploy7_2026-09-20.md` — Deploy #7 filing: 25adadd + 6540d3c (BELT-HISTORY slice 1 commits 1–2) to PA at 4e72bab; steps 0–11 PASS; runtime set N=1 game_bridge.py; SHA gate byte-identical (sha256 38b1de64…5d130); IMPORT-PATH-PROOF confirms cage_dynasty_web/ resolution and inertness of the root fight_engine.py ghost; DEPLOY-GATE-REF1 (a) invoked; error-log delta empty; Van's PA browser check PASS (2026-09-20)
- `claude/deploy8_2026-09-22.md` — Deploy #8 filing: 0c4bfb6 (HOF-REPOINT1, BELT-HISTORY slice 1 commit 3) + four docs commits (0e949fd, 330cb06, 39ca063, 20b828a) to PA at 20b828a; steps 0–11 PASS, Van's PA browser check PASS (2026-09-21); runtime set N=1 game_bridge.py; SHA gate byte-identical (sha256 c6604c2b…994af); IMPORT-PATH-PROOF confirms cage_dynasty_web/ resolution (fight_engine + fight_integration + commentary); error-log delta zero on timestamp test (baseline gap §4a); four v1.2 candidates added (§5); Slice 1 (BELT-HISTORY reader unification) complete on PA (2026-09-22)
- claude/handoff_post_deploy7.md — post deploy #7 handoff; supersedes handoff_post_slice1; ruling (a), PA anchor 4e72bab, fight_engine.__file__ measurement, sequencing to slice 1 commit 3 (2026-09-20)
- `claude/deploy_procedure.md` — disk-canonical deploy procedure v1.1 (cf201ee); supersedes deploy5 §1 text; diff-driven ship-file hash gate against `git show newPA:path`, import-path gate from server.log (2026-09-18)
- `claude/belt_history_gate0_2026-09-18.md` — BELT-HISTORY Gate 0 filing: pre-gen writes _belt_history only (world_init.py:3588); live transfers write both; Champions page + record book + HOF score read _title_history and miss pre-gen; profile reads _belt_history and renders correctly (Liu Xiaolong control); §6 corrections layer (Explore-agent phantom :9907, count slip); Van ruling single canonical store _belt_history (2026-09-18)
...
- `claude/golden_master_oracle.md` — GOLDEN MASTER coverage gap (moved from CLAUDE.md L92–199 at cd6f7e80, ARCHIVE4, 2026-09-22) <!-- ARCHIVE4 -->
- `claude/local_playtest_command.md` — local playtest command + PYTHONPATH (moved from CLAUDE.md L959–988 at cd6f7e80, ARCHIVE4, 2026-09-22) <!-- ARCHIVE4 -->
- `claude/claude_md_archive_2026c.md` — OWED ITEMS + 5c1477d resolution (moved from CLAUDE.md L989–1008 at cd6f7e80, ARCHIVE4, 2026-09-22) <!-- ARCHIVE4 -->
- `claude/certified_cell_baselines.md` — certified cell baselines symmetric skill (moved from CLAUDE.md L1010–1148 at cd6f7e80, ARCHIVE4, 2026-09-22) <!-- ARCHIVE4 -->
