...
class HistorySimulator:
    ...
    def _fighter_to_attributes(self, fighter: GeneratedFighter) -> Optional[Any]:
        ...
        try:
            # P3-4d: power passthrough with strength-derived fallback
            # (matches load-time derivation shape at game_bridge.
            # _a_power_derived — fighters generated by pre-C23
            # world_init.generate_attributes DO have a 'power' key at
            # generate_attributes:1001, but the fallback keeps a legacy
            # attributes dict without power viable via strength).
            _pow_val = fighter.attributes.get(
                "power", fighter.attributes.get("strength", 65))
    ...
    def _add_fighter_to_state(self, fighter: GeneratedFighter) -> None:
        ...
        if hasattr(self.game_state, '_fighter_data') and fighter.attributes:
            ...
            for _key, _val in fighter.attributes.items():
                # P3-4d: 'power' is now a canonical stat — was dropped
                # pre-C23 as "no canonical analog" but is the D7 lane
                # split target. Passes through here alongside the
