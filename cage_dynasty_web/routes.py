...
def _enrich_market_entry(c):
    ...
    Ship Coach-3 — surfaces type_name/type_icon from COACH_TYPES.
    COACH-COVERAGE1b — 'trained_stats' derived from the actual training
    bucket (SPECIALTY_MAP→_COACH_ATTRS) instead of COACH_TYPES.primary/
    secondary, so the card advertises what the coach really trains.
    Style-match/flavor kept via type_name/type_desc/style_match for
    visual distinction between coaches sharing a training bucket."""
    ...
    from game_bridge import COACH_TYPES, get_coach_trained_stats
    ...
    # COACH-COVERAGE1b: derive the trained-stats list from the
    # training bucket the coach's specialty resolves to. This is
    # what the coach ACTUALLY trains during weekly sessions — not
    # the aspirational COACH_TYPES.primary/secondary lists that
    # didn't match the bucket's contents.
    specialty = c.get('specialty') or ct_key
    trained_stats = get_coach_trained_stats(specialty)
    return {
        **c,
        'id':              c.get('coach_id'),
        'label':           label,
        ...
        # COACH-COVERAGE1b — honest stats list. Kept the old
        # primary_stats/secondary_stats keys pointed at the same
        # trained_stats list to avoid breaking any consumer that
        # already reads them; templates now render trained_stats.
        'trained_stats':   trained_stats,
        'primary_stats':   trained_stats,
        'secondary_stats': [],
        'style_match':     list(ct_def.get('style_match', [])),
    }
...
def _generate_available_coaches(tier):
    ...
    Post-COACH-COVERAGE1a: bumped num_coaches to 7 (matches full
    canonical type coverage from game_start.generate_starting_coaches);
    dropped the legacy-string half of _SPEC_LABEL and _CANON since
    game_start no longer emits those specialty strings.
    """
    try:
        from systems.game_start import generate_starting_coaches
        coaches = generate_starting_coaches(num_coaches=7)
        # Canonical coach_type → (display label, icon). game_start
        # now emits ONLY canonical keys, so the legacy-string half of
        # this table was dead code and has been removed.
        _SPEC_LABEL = {
        ...
        for c in coaches:
            ...
            # spec is now always a canonical coach_type key from
            # game_start's post-COACH-COVERAGE1a generator, so it
            # looks up directly in _STYLE_FIT with no legacy mapping.
            _canon_key = spec if spec in _STYLE_FIT else "sc_coach"
            ...
            result.append({
                ...
                "description":   (
                    f"{getattr(c, 'personality_desc', '')}. "
                    f"{c.get_bonus_description()}").lstrip('. '),
                "traits":        _traits,
                "trait_display": [_TRAIT_DISPLAY.get(t, (t.replace('_',' ').title(), 'personality'))
                                  for t in _traits],
        ...
        # COACH-COVERAGE1a: the previous inline fallback (a 130-line
        # COACH_TYPES_SETUP + tier-rating generation block) was dead
        # code — the try import above always succeeds in a live tree.
        # Deleted to reduce noise. If systems.game_start ever genuinely
        # fails to import, the mid-game hire market surfaces empty
        # rather than falling through to a stale duplicate generator.
