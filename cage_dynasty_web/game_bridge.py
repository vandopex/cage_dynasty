...
class GameBridge:
    ...
    def _convert_mock_fighter(self, f) -> WebFighter:
        ...
        _wf = WebFighter(
            ...
            ko_wins=f.ko_wins,
            sub_wins=f.sub_wins,
            fights_total=int(f.wins + f.losses + getattr(f, 'draws', 0)),
            ovr_at_signing=int(getattr(f, 'overall_rating', 0)),
    ...
    def _convert_amateur_fighter(self, amateur) -> WebFighter:
        ...
        _wf = WebFighter(
            ...
            sub_wins=0,
            fights_total=0,
            ovr_at_signing=int(getattr(amateur, 'overall_rating', 60)),
    ...
    _EQUIPMENT_TYPES = {
        ...
        "thai_pads":        {"name": "Thai Pads",       "icon": "🦵",
                             "domain": "clinch",
                             "desc": "Clinch striking + grip control work"},
        ...
        for wc in divisions:
            ...
            if FIGHT_ENGINE_AVAILABLE:
                try:
                    ...
                    # Store commentary for watch_fight page
                    # NOTE: in _simulate_ai_fights_week there's no `fight`
                    # dict in scope. Build the id from f1/f2/week — MUST
                    # match the result dict format used below (lines
                    # ~10327 and ~10392: "ai_fight_{week}_{f1}_{f2}").
                    try:
                        _ai_fight_id = f"ai_fight_{week}_{f1.fighter_id}_{f2.fighter_id}"
                        if _ai_fight_id and _ai_fight_id not in self._fight_commentary:
                            _ai_lines = []
                            if hasattr(_eng, 'full_commentary') and _eng.full_commentary:
                                _ai_lines = [l for l in _eng.full_commentary.split('\n') if l.strip()]
