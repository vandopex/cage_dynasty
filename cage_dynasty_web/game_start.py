...
POTENTIAL_GRADES = {
    "Elite": (90, 97),      # Future champion — rare, genuinely A/A+
    "High":  (79, 87),      # Contender potential — B+ to A-
    "Average": (68, 77),    # Solid roster fighter — B- to B
    "Limited": (55, 66),    # Journeyman ceiling — C to C+
}
...
def generate_starting_prospects(
    ...
    # Distribute potential grades across 9 weight classes:
    # Realistic scarcity — most prospects are average, top talent is rare
    # 0-1 Elite, 2 High, 4 Average, 2 Limited
    import random as _rand
    _elite_count = 1 if _rand.random() < 0.55 else 0   # ~55% chance of 1 Elite in the pool
    _high_count  = 2
    _avg_count   = 9 - _elite_count - _high_count - 2
    _lim_count   = 2
    potential_distribution = (["Elite"] * _elite_count +
                               ["High"]    * _high_count  +
                               ["Average"] * _avg_count   +
                               ["Limited"] * _lim_count)
    _rand.shuffle(potential_distribution)
