...
- **DRAW-SEMANTICS1** — UPGRADED 2026-09-16 (deploy #6 browser session). Prior filing (from FOUNDING-ROW2's lose_streak gate, 2026-09-15) recorded `result='L'` for BOTH fighters on a draw sample (Emil Lindqvist row, canonical dump). Van's browser session found the OPPOSITE direction on the CANONICAL live surface: Emeka Usman fight history row shows `result=WIN, method=DRAW`, Cage Dynasty 46 vs Charles Baker; record 4-3-0 (zero draws in header); finish breakdown reports "By DEC 3" against two actual decision wins — the draw counts under DEC WINS. So a draw likely SPLITS into W for one side and L for the other. The two samples CONTRADICT: Lindqvist vs Suzuki (CD15, dump) carried result='L' on BOTH rows; Usman vs Baker (CD46, browser) carried result='W' on Usman's row. Same pre-gen path, two encodings. Resolving that is Gate 0 — print Baker's row and the draw write path before scoping anything. No fighter on any player-visible surface in a fresh save carries a draw in their record; the D column appears permanently zero. Also `most_fights = wins + losses` (record book, game_bridge.py:14822) omits draws: 8-2-2 shows 10 fights. North-star hit: a career arc of "loss, draw called Battle of the Ages, another loss" reads to the engine as split W/L across fighters; every downstream result-keyed reader mis-counts. Same shape as FOUNDING-ROW2 — one encoding decision leaking into every downstream reader. Engine-tier, write-path, forward-only. Gate 0: grep every `result == 'L'` and `result != 'W'` reader; enumerate which player-visible surfaces / morale / news / matchmaker paths mis-count draws. Filed as observation; not addressed. Detail: `claude/deploy6_2026-09-16.md` §5.
