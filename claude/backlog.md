...
- **DRAW-SEMANTICS1** — NEW, surfaced 2026-09-15 during FOUNDING-ROW2's lose_streak gate. A draw fight writes `result='L'` for BOTH fighters (method='DRAW', specialty_method='Draw'; verified on canonical dump: Emil Lindqvist vs Daisuke Suzuki at Cage Dynasty 15, both rows carry `result='L'`). Consequence: every result-keyed reader treats a draw as a defeat — `_get_fighter_lose_streak`, the inline lose_streak in `_convert_real_fighter`, morale/comeback triggers that key on `result=='L'`. A fighter at 0-0-2 carries a 2-fight losing streak in the engine's reading. Also `most_fights = wins + losses` (record book, game_bridge.py:14822) omits draws: 8-2-2 shows 10 fights. North-star hit: a career arc of "loss, draw called Battle of the Ages, another loss" reads to the engine as three straight losses, which every downstream story-beat consumer inherits. Same shape as FOUNDING-ROW2 — one encoding decision leaking into every downstream reader. Gate 0: grep every `result == 'L'` and `result != 'W'` reader; enumerate which player-visible surfaces / morale / news / matchmaker paths mis-count draws. Filed as observation; not addressed in this ship. Detail source: FOUNDING-ROW2 filing (to be written).
