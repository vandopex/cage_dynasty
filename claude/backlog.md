...
- **WEEK-AXIS1** — (a) STAMP dcee687, (b) SEED 266e382 + BOOKER1 1ccefa0 + CHAMP-A 23c7599 SHIPPED LOCALLY (2026-09-10), not on PA. R06 seam 8→7→2→0, each step predicted by count and name; T-4 0; pre-gen 0 diffs. Filing `claude/week_axis1_b_filing_2026-09-10.md`. NEXT: (c) PREDICATE — division-gated + unified event_number compare on fighter_profile.html; NOT template-only (needs get_fighter_reigns serializer, instrument mirror α, and _convert_real_fighter shape). Then deploy #4; then (d) _recently_fought (measure first). FOUNDING-ROW2 open.
