...
- **WEEK-AXIS1** — GATE 0 DONE 2026-09-09 (filing `claude/week_axis1_gate0_2026-09-09.md`): _fighter_cooldowns MEASURED empty at week 0 — world-gen writes nothing, every fighter available at live wk 1 (R06 seam 8 = missing init). Sole R03 hit is the template's raw compare on a Founding reign. Ships as four commits, forward-only: (a) STAMP event_number on the three live writes via _dfc_label arithmetic (not week+60); (b) SEED cooldowns at new_game, (last pre-gen wk − 60) + COOLDOWN_WINNER, cooldowns only; (c) template PREDICATE on the unified axis; (d) _recently_fought measured then fixed. FOUNDING-ROW2 filed as open question (row-in-history is output-side).
