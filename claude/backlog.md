...
- **PLAYER-CREATE1 findings, none fixed** — PROSPECT-BONUS1 (5/9 draft prospects derive Balanced; balance-touching), OVR-FORMULA1 (AI OVR frozen at unweighted mean, player on _compute_ovr; pool bias −1.6), SETUP-OVR1, HARNESS-RNG1 (only the instrument's numbers are quoted), stale systems/game_start.py, G&P commentary string. Detail: filing §5. **OVR-FORMULA1 amendment (Van browser session 2026-09-15, post-deploy #5):** the ladder decorrelation is player-visible without any measurement. Lightweight ladder: champion 87 OVR, #1 75, #2 62, #3 80, #10 47, #11 70 — rating and rank are nearly uncorrelated on the page a player actually reads. Bantamweight: champion 79, the #1 who lost the belt to him 86 (successor rated lower than the fighter he beat for the belt). Same root as the filed north-star gap (AI OVR frozen at world-gen) seen from the reader's side, not the harness's.
...
- **PREGEN-SPAN1** (queued after REMATCH-COOLDOWN1, own commit) — persist the pre-gen week span as its own bridge attribute (e.g., `self._pregen_span_weeks = _initializer.history_weeks` in new_game near :2388 alongside `_dfc_event_offset`), plus web_save/web_load round-trip and legacy-save fallback (missing key → derive from `_dfc_event_offset` under the current 1:1 pre-gen cadence assumption). Then `_weeks_since_fought` uses `_pregen_span_weeks` for the week SPAN and `_dfc_event_offset` only to classify the axis — two constants for two jobs. Retires the per-row `en != fw` guard filed under REMATCH-COOLDOWN1 (guard becomes redundant once span is authoritative). Filing pointer: `claude/rematch_cooldown1_filing_2026-09-14.md`.
...
- **LINEAGE1** — R05 FAIL 2 (Middleweight: reign[0].lost_to != reign[1].champion_id) in dump_stamp_a, uuidseed_1, hs0_1; PASS in dump_20260908, base_rerun, uuidseed_2, hs0_2 — identical code. A live belt transition INTERMITTENTLY writes an inconsistent lineage. "Engine bug, not harness noise" DOUBTFUL (2026-09-14, REMATCH-COOLDOWN1 ship): R05 sorts reigns per-division by `won_week` at `save_invariants.py:481`, a split-axis field — pre-gen reigns carry won_week 0..history_weeks, live-play reigns restart at won_week 1..N. All 6 violations across 3 dumps in the REMATCH-COOLDOWN1 arc pair a pre-gen-axis reign (won_week 0-19) with a live-axis reign (won_week 1-11), ordered wrong on the unified timeline. Count wanders 4 / 2 / 2 across post-fix run 1 / post-fix run 2 / pre-fix, and the wander lands on different divisions (Featherweight+Middleweight, Strawweight, Middleweight) — consistent with a comparator artifact keyed on which reigns happen to fall in which axis slot. NEW Gate 0: fix the comparator's sort to unified weeks (or split-axis + independent check) and re-measure BEFORE hunting an engine write path. The harness only makes it visible; that "visible" may be the comparator, not the engine. Original Gate 0 (diff FAIL vs PASS Middleweight belt_history) preserved as a fallback if the comparator fix leaves violations. Detail: `claude/rematch_cooldown1_filing_2026-09-14.md` §5.
...
- **RECORD-BOOK1** — deploy #4 browser check: on a fresh save with 9 founding reigns and ≥2 title changes, the "Most Title Reigns" and "Most Defenses" record-book sections render EMPTY while other sections populate. CONFIRMED on a second save (Van's post-deploy #5 browser session, 2026-09-15) — Jingliang Li's profile shows a founding Bantamweight reign, 1 defense, lost at CD 18, so the data provably exists but the record-book render doesn't reach it. NEW DETAIL: two different code paths, not one missing source. "Most Title Reigns" and "Most Defenses" render BLANK — no rows AND no empty-state string — while "Most Sig Strikes" / "Most Sub Attempts" / "Most Takedowns" correctly print "No records yet" at week 0. Blank vs "no records yet" means the two sections fail differently: the stat sections have a working reader that finds no rows and renders the empty-state; the title-reign/defense sections don't reach either their reader or their empty-state at all. Gate 0 unchanged (`grep -n "record_book\|Most Title Reigns\|Most Defenses" cage_dynasty_web/routes.py cage_dynasty_web/templates/records.html`), but now the fix touches two render paths, not one. Not fixed here. RULING (Van, 2026-09-18): a founding champion's awarded reign counts as a title reign in the record book and nothing else — not a win, not a defense, not a streak link. "Most Title Reigns" counts a founder at 1; "Most Defenses" counts post-crown title wins only; a founder who never defended shows 1 reign / 0 defenses. Consistent with FOUNDING-ROW2 (shipped 46c094f). Gates the BELT-HISTORY arc (RECORD-BOOK1 + BELT-LINEAGE1 + VACATED-REIGN1). The arc's tier is a Gate 0 OUTPUT, not an input: whether live-play title transfers reach both stores decides read-side vs write-path. Filed inline per the ruling-open note in claude/handoff_post_deploy6.md. ADDRESSED 6540d3c (2026-09-19): _get_title_records reads _belt_history; both cards populate at week 0 in harness. CLOSES on Van's PA browser check at deploy #7 — fresh save, week 0, both cards non-blank. CLOSED 2026-09-20 (display, deploy #7 PA browser PASS, filing deploy7_2026-09-20.md §k). Van's browser: 'Most Title Reigns' + 'Most Defenses' cards both populated at week 0; Teixeira 2x reigns cross-checks against Champions page lineage.
...
- **FOUNDER-LABEL1** — NEW 2026-09-19 (PA browser session). Profile reign card renders 'Won from Inaugural · Cage Dynasty Founding — <div> Championship · Inaugural Crown' for founders; won_from_name is None on founding reigns, so the template is substituting 'Inaugural' into the won-from slot. Template-only, fighter_profile.html. Once Champions page renders founders (25adadd) the same wording question applies there. One-line fix, own commit, not scoped.
...
- **TITLE-CADENCE1** (observation, measurement attached 2026-09-14) — a champion skipped for cooldown at wk 2 was not rebooked until wk 11; planned builder fires weeks 1-3 only, then lead-time booking (10-12w). Live title fights in 14 weeks 9→6 after CHAMP-A. REMATCH-COOLDOWN1 ship (eeff834): title fights 14w moved 6 → 10, stable across HARNESS-RNG2 (both runs of the census, seed 20260907). 10 OVERSHOOTS the pre-CHAMP-A 9 that CHAMP-A deliberately took to 6. Not tuned inside the fix on purpose — a threshold change bundled into a bug fix hides the engine behind the output. Own docket; scope the direction (settle to something between 6 and 10, or accept 10) before touching a threshold. Detail: `claude/rematch_cooldown1_filing_2026-09-14.md` §5.
...
- **god-stat + nationality census** — playtest notes #6+#7: 11-of-19 stats at 95 on FLW champ (88 OVR); three fighters all "Brazil" nationality suggests country distribution may cluster (measure across saves before scoping). Height/reach clause promoted to HEIGHT-REACH1 (below); see that entry for the uniform-value finding + reconfirmation.
- **HEIGHT-REACH1** — height + reach are uniform across every fighter and every division. RECONFIRMED (Van browser session 2026-09-15, post-deploy #5) across two saves and every division sampled: bantamweight through heavyweight all render 5'10" / 72". Doubled-quote render on reach (`72""`) also persists. `world_init` never assigns per-fighter height/reach; the "Brazil, 5'10\", 72\"" trio in the original playtest note was not a sampling accident. Not fixed here.
- **PREGEN-BOOKING-SKEW1** — NEW, Van browser session 2026-09-15. In two saves on different seeds, the Record Book's "Most Fights" top five came ENTIRELY from ONE division — all Welterweight in the deploy-#5 save (2026-09-15), all Strawweight in the deploy-#4 save (2026-09-12). Different division each save, so this is not a fixed iteration order or a division-name filter. "Most Wins" and "Most Submissions" are spread across divisions in the same saves, so the skew is in BOOKING VOLUME, not fight outcomes — consistent with the pre-gen builder giving one weight class far more slots per event than the other eight. Player-visible on the record-book page (top-fights list looks broken because one division dominates it). Gate 0: count pre-gen fights per division across 2-3 seeds; print what selects the hot division (e.g., division rotation off a seeded index vs uniform-per-week). Not fixed here.
...
- **HANDOFF-TRANSPORT1** — 51 filings live in the claude.ai project store with no verified repo copy. Project store is write-only in practice: API-written docs have no file-upload entry and no download path; the only readback is architect-thread transcription (proven 2026-09-15 at 7d789d0). Decide: reconcile (transcribe each, hash-file per copy, one commit per batch) or accept them as project-only history. Forward-only — no bulk backfill without a per-file hash gate, since transcription is the only available transport and it cannot self-verify.
- **INDEX-COMPLETENESS1** — 21 files under `claude/` are absent from CLAUDE.md's library index (measured 2026-09-15): deploy5_2026-09-15, handoff_post_rematch_cooldown1, fight_model_p3_scope_v0_1 (the disk-canonical P3 board, peer-declared in backlog.md line 3), the six fight_model_* docs, three handoff_*, three stamina_* scope docs, three dated notes docs, two specs, post_c13_sequencing. cc Reads filings on demand via the index, so an unindexed filing is unreachable to it. This already produced a wrong call: a CLAUDE.md grep for fight_model_p3_scope_v0_1 returned zero hits and was read as the reference being stale, when the cause was the index gap. Decide whether the index is a manifest (index everything, add a pre-commit check) or a curated sample (say so in its header) — and state the ordering rule or record that there is none. Forward-only.
- **DRAW-SEMANTICS1** — UPGRADED 2026-09-16 (deploy #6 browser session). Prior filing (from FOUNDING-ROW2's lose_streak gate, 2026-09-15) recorded `result='L'` for BOTH fighters on a draw sample (Emil Lindqvist row, canonical dump). Van's browser session found the OPPOSITE direction on the CANONICAL live surface: Emeka Usman fight history row shows `result=WIN, method=DRAW`, Cage Dynasty 46 vs Charles Baker; record 4-3-0 (zero draws in header); finish breakdown reports "By DEC 3" against two actual decision wins — the draw counts under DEC WINS. The two samples CONTRADICT: Lindqvist vs Suzuki (CD15, dump) carried result='L' on BOTH rows; Usman vs Baker (CD46, browser) carried result='W' on Usman's row. Same pre-gen path, two encodings. Resolving that is Gate 0 — print Baker's row and the draw write path before scoping anything. No fighter on any player-visible surface in a fresh save carries a draw in their record; the D column appears permanently zero. Also `most_fights = wins + losses` (record book, game_bridge.py:14822) omits draws: 8-2-2 shows 10 fights. North-star hit: a career arc of "loss, draw called Battle of the Ages, another loss" reads to the engine as a loss for both fighters or a win for one, depending on which encoding fired — never as a draw; every downstream result-keyed reader mis-counts. Same shape as FOUNDING-ROW2 — one encoding decision leaking into every downstream reader. Engine-tier, write-path, forward-only. Gate 0: grep every `result == 'L'` and `result != 'W'` reader; enumerate which player-visible surfaces / morale / news / matchmaker paths mis-count draws. Filed as observation; not addressed. Detail: `claude/deploy6_2026-09-16.md` §5.
- **BELT-LINEAGE1** — NEW, deploy #6 browser check (2026-09-16). Champions page Lightweight lineage reads "0 Title Changes / 1 Total Reign" and labels Machado "Inaugural Champion at Season Opener" — but Machado's own profile shows he won the belt from Steven Thompson at CD59. Thompson's reign is invisible on the Champions page. Same two-store shape suspected as RECORD-BOOK1 (blank record-book sections) and VACATED-REIGN1 (belt_history vs _title_history diverge on vacates). Three-in-one arc candidate: consolidate RECORD-BOOK1 + VACATED-REIGN1 + BELT-LINEAGE1 under one belt-history-render unification. Detail: `claude/deploy6_2026-09-16.md` §5. Gate 0 (2026-09-18, claude/belt_history_gate0_2026-09-18.md): pre-gen writes _belt_history only (world_init.py:3588); live transfers write both (game_bridge.py:15801-15900); Champions page (:15907), record book (:15112) AND HOF score (:4393) read _title_history and never see a pre-gen reign — every pre-gen champion has been HOF-scored at zero reigns. Machado at CD59 was the pre-gen champion at kickoff; "Inaugural at Season Opener" is the synthetic seed at :15910-15929 firing on an empty store. Profile reads _belt_history and renders pre-gen + live correctly (Liu Xiaolong control). RECORD-BOOK1 blank-at-week-0 is the same empty store; moot once populated. RULED (Van, 2026-09-18): single canonical store, _belt_history. Slice 1 = three single-purpose repoint commits (two display tier, HOF engine tier with before/after HOF measurement); slice 2 = VACATED-REIGN1 vacate_reign on BeltHistory, both vacate sites (:4058, :6939) call it; slice 3 = retire _title_history writes once no reader remains. RULED (Van, 2026-09-18): yes — a founding reign is a reign for HOF, same as the record book. The 25/reign coefficient is UNTOUCHED in the repoint commit; whether it is right is unmeasured (every HOF score to date counted pre-gen reigns at zero). The repoint's before/after measurement — inductees crossing 60 before vs after on a fresh world advanced one season, and each inductee's reign-share of score — is what decides whether a HOF-TUNE docket exists. SLICE 1 PROGRESS (2026-09-19): commit 1 CHAMPIONS-PAGE1 25adadd, commit 2 RECORD-BOOK-REPOINT1 6540d3c — both pushed, NOT on PA (PA at 46c094f). Commit 3 (_compute_hof_score, engine tier, before/after HOF induction measurement) OWED. Before-evidence on PA fresh save 2026-09-19: Gadjiev profile shows two Featherweight reigns (founder, 6 defenses, lost CD42, regained CD47, 1 defense) while Champions page shows one reign 'Inaugural at Season Opener', 0 defenses; Lopez 7 defenses on profile, 0 on lineage. Harness after-evidence seed 20260918: Adesanya two Bantamweight reigns (won CD16, lost CD51, regained CD58) now tops Most Title Reigns; Garcia two Middleweight reigns on lineage. Deploy #7 carries both commits; runtime diff set = cage_dynasty_web/game_bridge.py only. DISPLAY HALF CLOSED 2026-09-20 (deploy #7 PA browser PASS, filing deploy7_2026-09-20.md §k). Van's browser: Champions/Lightweight chains Teixeira founding → Allen → Teixeira current, no 'Season Opener,' 3 total reigns; cross-checks against record book. HOF-reader half (_compute_hof_score at :4388) remains open — closes on BELT-HISTORY slice 1 commit 3. Slice 1 COMPLETE 2026-09-21 (CHAMPIONS-PAGE1, RECORD-BOOK-REPOINT1, HOF-REPOINT1 — see git log); not deployed. Next: slice 2 VACATED-REIGN1 (BeltHistory.vacate_reign; vacate sites :4058, :6939), then slice 3 retire _title_history writes. Filing: handoff_post_slice1.md §7.
- **RECAP-STALE1** — NEW, deploy #6 browser check (2026-09-16). Week 4's weekend recap re-displayed Week 2's card verbatim — CD62, Tashkent, all eight fights, same results. CD63 never appeared; CD64 showed at week 6. Finance ledger confirms weeks advanced. Isolated to the recap render, not the underlying week counter. Player-visible template/display issue. Gate 0: find the recap render path (grep for "weekend recap" / "recap" in routes.py + templates); print whether it reads a stored last-recap variable or computes from the current week index. Detail: `claude/deploy6_2026-09-16.md` §5.
- **HISTORY-CAP1** — NEW, deploy #6 browser check (2026-09-16). Fight history appears capped at 10 rows on the profile page. Tsarukyan profile states "11 fights" in the record header but lists only 10 rows in the history table. His first reign claims 6 defenses against 5 visible wins in that window. Display-tier cap suspected. Gate 0: grep templates/fighter_profile.html for `[:10]` / `limit(10)` / any Jinja slice on fight_history. Detail: `claude/deploy6_2026-09-16.md` §5.
- **GOAT-SORT1** — NEW 2026-09-20 (deploy #7 PA browser session). Rankings GOAT top 10 display order does not follow displayed pts (93, 93, 90, 95, 98, 78, 55, 65, 71, 83) or wins; identify sort key; pre-existing vs 46c094f..4e72bab unknown — Gate 0 is `git diff 46c094f..4e72bab -- cage_dynasty_web/game_bridge.py | grep -i goat`.
- **HOF-TUNE1** — NEW 2026-09-21 (HOF-REPOINT1 harness, seed 20260921 week 26): 56/290 fighters ≥ HOF_THRESHOLD 60 with 43 of them holding zero reigns in either store; only 7 retired. Threshold is the lever, not 25/reign. Do not tune until a multi-season world is measured (year-end sweep never ran at week 26). Detail: handoff_post_slice1.md §7.
- **TITLE-KEYS1** — NEW 2026-09-21: belt store 9 keys vs _title_history 8 keys (missing Heavyweight) in the seed-20260921 week-26 save; belt initialized eagerly at world_init.py:611, _title_history lazy at game_bridge.py:15807-15808 on first write. Writer guards missing keys at :15807; readers at :4058/:6940 use `.get(wc, [])` — no crash hazard. Write function `_record_title_result` at :15765: key-init at :15807-15808 fires on any call (change OR defense); defense branch at :15882 only mutates `history[-1]["successful_defenses"]` if `history and history[-1].get("is_active")`. A live defense by a pre-gen champion (whose reign lives only in _belt_history) therefore inits the key without appending — the 8-empty-lists fingerprint. If defense writes init the key without a row, _title_history drops every defense by a pre-gen champion until slice 3 — the record-book defenses gap by another route.
- **RECORD-BOOK-STATS1** — NEW 2026-09-20 (deploy #7 PA browser session). Duplicate of playtest note #10 (pre-gen fights carry no stat keys) and the 2026-07-11 reconciled-closed entry. Single open design question: should pre-gen rows carry per-round stats? Decide after slice 3.
- **DEPLOY-GATE-REF1** — NEW 2026-09-18 (post-deploy #6 read-only audit). Three defects in the standing deploy procedure, all measured, none of which bit deploys 5 or 6 — verified retroactively: `git show 46c094f:cage_dynasty_web/game_bridge.py | shasum -a 256` = 106e11af… (matches deploy6 filing L42-43); `git show 6d7abc2:…` = f8a684d3… (matches deploy5 L166-167). (1) REFERENCE: spec (deploy5 §1 L60-61) says hash "the local committed copy"; practice (deploy5 L167, deploy6 L43) hashes the working-tree path. A local uncommitted edit would pass the gate vacuously. Forward rule: hash `git show <newPA>:<path>`, never a working-tree path. (2) SCOPE: the file hashed is chosen by habit (game_bridge.py), not by the diff. Correct-by-coincidence for both deploys (46c094f and 9faeba8 each touched game_bridge.py as their only runtime file). Forward rule: hash every runtime file in `git diff --name-only <prevPA>..<newPA>` (exclude claude/, *.md); docs-only deploys hash zero runtime files and the pull-transport check (game_bridge.py) becomes its own explicitly-labelled line. (3) IMPORT-PATH never gated: CLAUDE.md L682-686 calls IMPORT-PATH-PROOF (db15e3a, 2026-07-12) "the standing regression test" for the dirty root fight_engine.py PA carries (` M fight_engine.py` in both deploy 5 and 6 pre-states). It has run once. `grep -i "import.path\|__file__" claude/deploy5_*.md claude/deploy6_*.md` → zero hits. Mechanism (game_bridge.py L218-247): module-load stderr print of `sys.modules["fight_engine"].__file__` and `fight_integration.__file__`, lands in server.log per uWSGI spawn. Forward rule: extend step 6c's existing server.log fetch — grep both `[IMPORT-PATH-PROOF]` lines from the POST-reload spawn and assert each equals its expected `/home/vandopegaming/cage_dynasty/cage_dynasty_web/…` path exactly. TRAP: the diagnostic prints ✅ with `?unknown?` when the module isn't in sys.modules under that name — assert the path, not the emoji. THEN, once the gate has passed in-app once: reconcile the ghost via console `git checkout HEAD -- fight_engine.py` (already prescribed by CLAUDE.md L687-699 as a Ship 1 prerequisite), and drop ` M fight_engine.py` from the expected pre-state. Keep `?? outputs/sm1/` as expected until DUMP-PROVENANCE1 rules. Procedure of record currently lives inside deploy5's filing (history, uneditable by rule) — needs a disk-canonical home: `claude/deploy_procedure.md`, indexed, filings point at it. Forward-only. Not engine-touching; the checkout on PA is a live-filesystem touch and stops before execution. v1 LANDED 9b69d3f (2026-09-18) at claude/deploy_procedure.md, indexed CLAUDE.md L1174; deploy #7 follows it. v1.2 candidates — see deploy7_2026-09-20.md §g, two items: (a) step 3 as written STOPs on any ` M` line, but ` M fight_engine.py` is the expected pre-state (deploys 5 and 6) until the checkout this docket prescribes AFTER step 10 — circular as written; reword so the ghost line is tolerated drift until retired and any other ` M` is a STOP. (b) The gate-count paragraph explains 10→13 but not deploy4's 12; state what is being counted. Steps 5 and 6 are actions, not gates: eleven gates, not thirteen. Deploy #8 (2026-09-22) added four v1.2 candidates — deploy8_2026-09-22.md §5; step 11 baseline gap bit (§4a).
- **NAME-COLLISION1** — NEW, deploy #6 browser check (2026-09-16). "Steven Thompson" and "Stephen Thompson" are distinct fighters, both in Machado's history four events apart. Two possibilities: intentional (real MMA has near-miss surnames) or name-generator bug (should dedup on edit distance). Confirm which is intended before scoping. Detail: `claude/deploy6_2026-09-16.md` §5.
- **DIVISION-GENDER1** — NEW, deploy #6 browser check (2026-09-16). Karolina Grabowski ranked #1 Welterweight, contested the title vs Piotr Jedrzejczyk. Mixed-gender division. Confirm whether intended before scoping any fix or new-division split. Detail: `claude/deploy6_2026-09-16.md` §5.
- **HARNESS-ENV1** — NEW, filed 2026-09-15 (surfaced during FOUNDING-ROW2 item 1 investigation). Under `save_invariants`'s `_harness_env`, bare `import fight_engine` resolves to `systems/fight_engine.py` (CLI dead copy) instead of `cage_dynasty_web/fight_engine.py` (web copy). Consequence today: `FULL_ENGINE_AVAILABLE=False` locally when running save_invariants; PA's wsgi.py setup puts `cage_dynasty_web/` at sys.path[0] and gets `FULL_ENGINE_AVAILABLE=True`. The `sys.path.insert(0, ...)` loop in `claude/tools/_harness_env.py` is not idempotent against pre-existing script-dir / PYTHONPATH entries; historic dumps in `outputs/sm_inv/` were generated when the environment resolved bare imports to the web copy (likely different invocation shell at generation time — PYTHONPATH, CWD, or `-m`). Not fixed. Existing dumps are real-engine (per DUMP-PROVENANCE1 sweep); new dumps produced by today's shell are fallback until fixed. Own docket. Detail: `claude/deploy6_2026-09-16.md` §5. Gate for a real-engine harness world is world_init.FULL_ENGINE_AVAILABLE, not fight_engine.__file__ — the latter passed on a fallback world 2026-09-18 while fight_integration's shim had latched onto systems/fight_engine.py. Workaround that worked: prepend cage_dynasty_web to sys.path[0] before any project import (outputs/belt_history/gate0_harness_v2.py).
- **DUMP-PROVENANCE1** — NEW, filed 2026-09-15 (surfaced during FOUNDING-ROW2 item 1 investigation). `save_invariants` dump format records `seed, weeks, mode` but not `generating commit`, `timestamp`, `command line`, or `FULL_ENGINE_AVAILABLE` state at generation. Measured on `outputs/sm_inv/dump_20260908.json`: top-level keys are `['belt_history', 'fighter_data', 'fighters', 'gen_fighters', 'meta', 'mode', 'player_camp_id', 'seed', 'weeks']`; `meta = {'week': 14}`. The DRAW/method-distribution test is the interim retroactive classifier — fallback worlds have 0 DRAWs and ~18% KO; real-engine worlds have DRAWs and KO ≪ 18%. Sweep 2026-09-15: 21 save_invariants dumps in `outputs/sm_inv/` real-engine; 4 `/tmp` session dumps real-engine; 2 `outputs/sm1/saveload1/scratch_saves/bridge_van_sl1t2_r*.json` fallback (predate `_harness_env`). Add generation-provenance field so future dumps carry it inline. Detail: `claude/deploy6_2026-09-16.md` §5. Gate 0 result (2026-09-18): outputs/ is 784M, 602M of it cited by filings; the running app does not write there; PA deploys via git pull. Wholesale tracking is out. Shape: a hash manifest generated from claude/*.md citations. Not scoped.
- **REMATCH-COOLDOWN1** — CLOSED 2026-09-16. Shipped `9faeba8` (2026-09-14 commit), ON PA at deploy #5 (2026-09-15 `6d7abc2`) and deploy #6 (2026-09-16 `46c094f`). Van's browser check on fresh save 2026-09-16: Tsarukyan def. Nicholas Bailey at CD62 (live-play, week 2) having beaten him at CD49 (pre-gen) — structurally impossible pre-`9faeba8` (raw `current_week - fought_week` returned large negative on pre-gen rows, cooldown gates blocked). Filings: `claude/rematch_cooldown1_filing_2026-09-14.md`, `claude/deploy5_2026-09-15.md`, `claude/deploy6_2026-09-16.md` §4. Owed browser check delivered; docket closed.
- **FOUNDING-ROW2** — CLOSED 2026-09-16. Shipped `46c094f` (2026-09-15 commit), ON PA at deploy #6 (2026-09-16). Canonical filtered streak walkers (`_win_streak_from_history` / `_longest_win_streak_from_history`) unify five consumers; retirement-news reader at :11416 switched to canonical (removes ratchet dependency). Van's browser check on fresh save 2026-09-16: Piotr Jedrzejczyk founding Welterweight champion 9-0-0 shows streak = 9 (pre-fix: 10); all five rows on both streak boards satisfy streak ≤ wins; Tsarukyan 10-1-0 correctly shows active=4 post-loss reset. Retirement-news reader fix means the same is true for any founding champion who retires — surfacing depends on retire trigger. Filing: `claude/deploy6_2026-09-16.md` §4. FOUNDING-ROW2 symptom (streak+1) row at L25 is the closed docket's genesis observation.
- **COOLDOWN-CONSTANTS1 / TITLE-CADENCE1 — corroboration 2026-09-16** (deploy #6 browser check). Piotr Jedrzejczyk's inaugural Welterweight reign shows 9 defenses across 9 pre-gen wins — every fight of his career was a title fight. Week 6's live card was #13 vs unranked main event with six unranked prelims, two weeks after two title fights. Consistent with the champion-metronome pattern COOLDOWN-CONSTANTS1 flagged and the "title cadence bunches then droughts" pattern TITLE-CADENCE1 captured. Not new; corroborates existing dockets. Detail: `claude/deploy6_2026-09-16.md` §5.
