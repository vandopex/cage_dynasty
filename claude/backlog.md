...
- **TOKEN-HYGIENE1** (sequenced with SECRET_KEY on PA) — deploy.sh carries two hardcoded literals: PA_TOKEN at :5 (revoked, dead) and DEPLOY_TOKEN at :8 (git-pull webhook; status unknown, treat as live); if deploy.sh is tracked both are in history. Fix: rotate DEPLOY_TOKEN; both read from chmod-600 files outside git; never echo either in a command line. Reload API returned 401 at deploy #3 (PA_TOKEN unset in both shells); Web-tab Reload used and recorded. Detail: `claude/deploy3_2026-09-08.md` §5.
- **WEEK-AXIS1** — (a) STAMP SHIPPED dcee687 (2026-09-10): live rows carry event_number parsed from their own event_name; T-4 202→0, name/number 1806/1806, pre-gen 0 diffs. (b) SEED BLOCKED on HARNESS-RNG2 — its gate (R06 seam 8→0) is a live-play count and live play is not reproducible across processes. (c) PREDICATE, (d) _recently_fought unchanged. Gate 0 filing `claude/week_axis1_gate0_2026-09-09.md`. FOUNDING-ROW2 open.
- **HARNESS-RNG2** (tools; BLOCKS WEEK-AXIS1 (b)) — MEASURED 2026-09-10: same code, seed 20260907 --weeks 14, two processes → 85/293 fighters with different history lengths, 120/1289 paired live rows differ; pre-gen rows 0/1618 differ. HARNESS-RNG1's "process-stable" finding was pre-gen only and is FALSE for live play. No live-play instrument count (R03/R05/R06) can gate a commit until two runs of the same code match. Suspect: uuid4 ignores the seed and steers iteration order somewhere on the live path (inference). Gate 0: seed uuid4 in _harness_env.py, run twice, diff; if identical, that is the fix and the seed-20260907 baseline moves again (dump_20260908 becomes history).
- **LINEAGE1** — R05 FAIL 2 in outputs/sm_inv/dump_stamp_a.json (Middleweight: reign[0].lost_to != reign[1].champion_id); PASS in dump_20260908 and dump_base_rerun. A live belt transition wrote an inconsistent lineage in that world. Not caused by STAMP (no live-path reader of event_number); reproducibility unknown until HARNESS-RNG2. Read the two Middleweight reigns in that dump before scoping.
- **ROW-SCHEMA1** — live fight_history rows write `round_finished`, pre-gen writes `round`; live rows omit `weight_class`. Two schemas on one list; anything walking history for round or weight class reads one of them. Forward-only unification, own commit.
