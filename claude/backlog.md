...
- **WEEK-AXIS1** — (a) STAMP dcee687, (b) SEED 266e382 + BOOKER1 1ccefa0 + CHAMP-A 23c7599, (c) PREDICATE ada3955 SHIPPED LOCALLY (2026-09-11), not on PA. R06 8→7→2→0 predicted by count and name ((b) only); R03 FP 0 in two runs + one confirm (before-count is process-dependent: 1 on dump_champ_2, 1/2/4/6/7 seen); T-4 0; pre-gen 0 diffs; render probe 0 violations across 3 fighters incl. multi-reign. Filings `claude/week_axis1_b_filing_2026-09-10.md` + `claude/week_axis1_c_filing_2026-09-11.md`. NEXT: deploy #4; then (d) _recently_fought (measure first). FOUNDING-ROW2 open. VACATED-REIGN1 filed (write-path fix, own docket).
