...
7. §6.5 CORRECTION (this file, added 2026-09-20). §6.5's "step 8 gates the whole prevPA..newPA set — 7 files" is architect-error-in-the-corrections-layer. Procedure v1.1 step 8 gates the **runtime set only** — `git diff --name-only prevPA..newPA` minus `claude/` and `*.md`. For deploy #7 (prevPA 46c094f → newPA 4e72bab) the runtime set is N=1: `cage_dynasty_web/game_bridge.py`. §6.5 body preserved unchanged; the correction layers here per verbatim-provenance rule. Filing: `claude/deploy7_2026-09-20.md` §i.
8. §6.6 SCOPE (added 2026-09-20). Item 6's List/Read substitution charges were read off collapsed fold labels and are unverified per instance; JSONL counts show Read tool calls do occur (96–263 per session) and List was not counted. The separate Grep-tool reading in deploy7 §j is false as written — see deploy7_2026-09-20.md §k.

## §7 — Commit 3: HOF-REPOINT1 (0c4bfb6, 2026-09-21)

_compute_hof_score reads _belt_history.get_fighter_reigns (the idiom profile paths :7103/:7298/:7580 already use); founding reigns count; _belt_history None raises RuntimeError (architect recommendation, shipped on Van's commit word — not separately ruled). Coefficients, threshold, callers untouched.

Gate: prediction-then-verify on one saved world. outputs/hof/hof_predict_v1.py — seed 20260921, new_game, 26 advance_week, web_save slot hof_harness (user_id hof_predict), computed per-fighter predicted score with the reign term recomputed from _belt_history. outputs/hof/hof_verify_v1.py — web_load same slot, real function post-edit: 290/290 fighters equal predicted, 0 mismatches, max |Δ| 0.0. Both harness files untracked under outputs/ (DUMP-PROVENANCE1).

Measured on that world: 290 fighters, 283 active. _belt_history 19 reigns (9 founding + 10 pre-gen transfers, event numbers 6–56 = won_week); _title_history 0 reigns. GAIN 15 fighters (belt-side reigns > title-side). REGRESS 0 — vacuous: title side empty, so live-mirror equivalence was NOT exercised; the _title_history live write is unexercised, not proven live or dead (inference from event numbering: no live title CHANGE in 26 weeks; live title DEFENSES likely did occur — see Keys). FLIPS 2 at threshold 60: Francis Saint Denis 58.2→143.2 (1 reign/6 def), Rafael Lopes 56.7→101.7 (1/2). Round-trip save/load: totals equal.

HOF-TUNE input: 56/290 already ≥60 at week 26, 43 of them with zero reigns in either store, 7 retired. Threshold, not 25/reign, is the lever. No tuning shipped.

Env: PYTHONPATH=narrative:systems:cage_dynasty_web plus the harness sys.path[0] prepend; FULL_ENGINE_AVAILABLE=True asserted.

Keys: belt store initialized eagerly to all 9 WEIGHT_CLASSES at world_init.py:611 (`{wc: [] for wc in WEIGHT_CLASSES}`); _title_history initialized as `{}` at game_bridge.py:2201 and lazy-populated per weight_class on first title-fight write at :15807-15808. In the hof_harness save at week 26: belt keys = 9, title keys = 8 (missing Heavyweight) despite pre-gen writing 5 Heavyweight belt reigns. Write function is `_record_title_result` at game_bridge.py:15765; the key-init at :15807-15808 fires on any call (change or defense); the defense branch at :15882 only mutates `history[-1]["successful_defenses"]` under `if history and history[-1].get("is_active")`. A live defense by a pre-gen champion whose reign lives only in _belt_history therefore inits the _title_history key without appending — matching the 8-empty-lists fingerprint exactly. Defense IS captured on the belt side via `_belt_history.record_title_defense(weight_class)` at :15898. No crash hazard: writer guards missing keys, readers at :4058/:6940 use `.get(wc, [])`.
